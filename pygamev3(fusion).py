...
rock_block = pygame.image.load("assets/Tiles/test_ting.png").convert_alpha()
...
selector = pygame.image.load("Inventory_bar_selector.png").convert_alpha()


#-------music---------------
pygame.mixer.music.load("assets/music/vibes.ogg")
laser_sound = pygame.mixer.Sound("assets/music/laser.ogg")
...
#-----------------------------------------------------------------------------------
    ...
    while run:
        ...
        e.handle_particles(particles, screen)
...
#----------------------------------------------------------------------------------------------------------------------------------------
        ...
        for chunk in game_map_for_stuff:
            for tile in game_map[chunk]:
                ...
                if player.selected_inventory_slot!=False and player.selected_inventory_slot!=True:
                    if player.selected_inventory_slot[0][1]==False:
                    ...
                    if tile_adjusted_rect.collidepoint(pygame.mouse.get_pos()) and mouse_click==True:
                        ...
                        if tile_distance_line<200:
                            ...
                            if tile[4]<0:
                                if tile[1]!=0:
                                    e.make_particles(particles, [tile_adjusted_rect.centerx, tile_adjusted_rect.centery], 1.5, [5,9], 100, 0.5, (135,128,128))
                                if tile[1] ==3:
    ...
    #------------------
        ...
        player.display_inventory( inventory_bar, screen, selector)
     ...
     #------------------------------------------------
        ...
        for enemy in enemies:
            if enemy.rect.colliderect(screen_rect):
                ...
                if enemy.health<0:
                    e.make_particles(particles, [enemy.rect.centerx-scroll[0], enemy.rect.centery-scroll[1]], 1, [3,7], 100, 0.5 , (255,10,0))
     ...
     #event handling ---------------------------------------------------------------
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key == K_1:
                    player.change_selected_inventory(0)
                if event.key == K_2:
                    player.change_selected_inventory(1)
                if event.key == K_3:
                    player.change_selected_inventory(2)         
                if event.key == K_4:
                    player.change_selected_inventory(3)
                if event.key == K_5:
                    player.change_selected_inventory(4)    
            ...
            if event.type == MOUSEBUTTONDOWN and event.button ==4:
                player.change_selected_inventory(False, 1)
